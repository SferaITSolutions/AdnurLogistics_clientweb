...
export const authService = {
  ...
  async sendCode(number: number) {
    return await axiosInstace.post(`/auth/send/code?phone=${number}`);
  },
  async resentPassword(data: {
    identity: string;
    password: string;
    resentPassword: string; 
  }) {
    return await axiosInstace.put(`/auth/resent/password`, data);
  },
  async refresh(token: string) {
    try {
      const response = await axiosInstace.post("/auth/refresh/token", {
        token,
      });
